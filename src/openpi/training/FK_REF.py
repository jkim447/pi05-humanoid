...
T_cam_to_base_right = np.array([
    [1.46041793e-04, -4.36539362e-01,  8.99685147e-01,  1.42125952e-01],
    [-9.99736362e-01,  2.05935101e-02,  1.01545313e-02, -3.41261053e-02],
    [-2.29605278e-02, -8.99449439e-01, -4.36421266e-01,  4.31371964e-01],
    [0.0, 0.0, 0.0, 1.0]
])
...
# Add rotation offset to left hand since assemble is not perfectly symmetric
angle_5 = np.deg2rad(-5) # TODO: change offset
c, s = np.cos(angle_5), np.sin(angle_5)
angle_10 = np.deg2rad(-10) # TODO: change offset
c10, s10 = np.cos(angle_10), np.sin(angle_10)
R_x = np.array([[1, 0,  0],
                [0, c, -s],
                [0, s,  c]])
R_y = np.array([[ c10, 0, s10],
                [ 0, 1, 0],
                [-s10, 0, c10]])
T_ee_to_real_ee = np.eye(4)
T_ee_to_real_ee[:3, :3] = R_x @ R_y



translation_offset_left = np.array([-0.02, -0.06, 0.0]) # TODO: change offset
translation_offset_right = np.array([0.0, 0.0, 0.04]) # TODO: change offset
T_real_ee_to_hand_left = np.eye(4)
T_real_ee_to_hand_left[:3, :3] = R_ee_to_hand_manual_left
T_real_ee_to_hand_left[:3, 3] = translation_offset_left
T_ee_to_hand_left = T_ee_to_real_ee @ T_real_ee_to_hand_left
